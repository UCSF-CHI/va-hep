...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="includeTableFooter" mode="includeTable">
    <p style="font-size: 12px; margin-top: -12px; margin-bottom: 25px; padding-top: 0;">
      <xsl:apply-templates />
    </p>
  </xsl:template>
  ...
  <xsl:template match="pony">
    ...
    <div id="{$ID}" class="slide">
      <xsl:if test="$POSITION != '1'">
        <xsl:attribute name="class">hide slide</xsl:attribute>
      </xsl:if>
  ...
  <xsl:template name="pony-link">
    ...
    <xsl:choose>
      <xsl:when test="$PDFREF != ''">
        <xsl:value-of select="$TSP" />
        pdf/
        <xsl:value-of select="$PDFREF" />
        .pdf
      </xsl:when>
      <xsl:when test="$XREF != ''">
        https://
      ...
      <xsl:otherwise>
        <xsl:call-template name="convertREF">
          <xsl:with-param name="FILE" select="$REF" />
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
      ...
      <xsl:choose>
        <xsl:when test="not($SLX) or $SLX = '1'">
          ...
          <xsl:if test="slide[@no='1']/sl-text">
            <div class="slidetext">
              <xsl:apply-templates select="slide[@no='1']/sl-text" />
            </div>
          </xsl:if>
          <xsl:choose>
  ...
  <xsl:template match="slide" mode="first">
    <xsl:variable name="newREF">
      <xsl:call-template name="convertPost">
        <xsl:with-param name="FILE" select="$XMLFILE" />
      </xsl:call-template>
    </xsl:variable>
    <p class="smslide">
  ...
  <xsl:template match="sec-imglib-slides">
    ...
    <xsl:if test="slide/@no = $SLX">
      <xsl:choose>
        <xsl:when test="$SLN/sl-title">
          <h2>
            <xsl:apply-templates select="$SLN/sl-title" />
          </h2>
        </xsl:when>
        <xsl:otherwise>
          <h2>
            <xsl:apply-templates select="$SLN/@title" />
          </h2>
        </xsl:otherwise>
      </xsl:choose>
    ...
    <p class="bylinedesc">
      <xsl:choose>
        <xsl:when test="$TS = 'true'">
          <a href="{$LINKPATH}provider/image-library/instructions.asp" title="How to download this image">Instructions</a>
          for downloading images.
        </xsl:when>
        <xsl:otherwise>
          <a href="va?page=we-teamsite&#38;TS=provider/image-library/instructions" title="How to download this image">Instructions</a>
          for downloading images.
        </xsl:otherwise>
      </xsl:choose>
    </p>
        ...
        <p>
          <xsl:choose>
            <xsl:when test="slide2/sl-title">
              <h2>
                <xsl:apply-templates select="slide2/sl-title" />
              </h2>
            </xsl:when>
            <xsl:otherwise>
              <h2>
                <xsl:apply-templates select="slide2/@title" />
              </h2>
            </xsl:otherwise>
          </xsl:choose>
        </p>
        ...
        <p>
          <xsl:choose>
            <xsl:when test="slide3/sl-title">
              <h2>
                <xsl:apply-templates select="slide3/sl-title" />
              </h2>
            </xsl:when>
            <xsl:otherwise>
              <h2>
                <xsl:apply-templates select="slide3/@title" />
              </h2>
            </xsl:otherwise>
          </xsl:choose>
        </p>
  ...
  <xsl:template match="sl-link">
    <xsl:if test=". != ''">
      <p>
        ...
        <br />
        <a href="http://hivinsite.ucsf.edu/InSite?page=kb-00&#38;doc={@ref}">
          <xsl:value-of select="." />
        </a>
      </p>
    </xsl:if>
  ...
  <xsl:template match="sec-imglib">
    ...
    <xsl:variable name="newREF">
      <xsl:choose>
        <xsl:when test="$TS = 'true'">
          <xsl:call-template name="convertPost">
            <xsl:with-param name="FILE" select="/pg/@id" />
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="convertPost">
            <xsl:with-param name="FILE" select="document($TSXML)/pg/@id" />
          </xsl:call-template>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
                ...
                <span>
                  <xsl:choose>
                    <xsl:when test="thumb/thumb-title">
                      ...
                      <xsl:apply-templates select="thumb/thumb-title" />
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:apply-templates select="new" />
                      <xsl:apply-templates select="sl-title" />
                    </xsl:otherwise>
                  </xsl:choose>
                </span>
              </p>
  ...
  <xsl:template match="thumb">
    ...
    <xsl:if test="@src2">
      <img src="{$TSP}{@src2}" width="50" height="50" border="1" alt="thumbnail image of {../slide2/sl-title}" title="thumbnail image of {../slide2/sl-title}" />
    </xsl:if>
    <xsl:if test="@src3">
      <img src="{$TSP}{@src3}" width="50" height="50" border="1" alt="thumbnail image of {../slide3/sl-title}" title="thumbnail image of {../slide3/sl-title}" />
    </xsl:if>
  </xsl:template>
          ...
          <li>
            <xsl:choose>
              <xsl:when test="$THUMBPAGE = '999'">
                <b>
                  View all (
                  <xsl:value-of select="$THUMBCOUNT" />
                  slides)
                </b>
              </xsl:when>
              <xsl:otherwise>
                <a href="{$newREF}thumbs=999">View all</a>
                (
                <xsl:value-of select="$THUMBCOUNT" />
                slides)
              </xsl:otherwise>
  ...
  <xsl:template match="popquiz">
    ...
    <div id="popquiz-{$qzno}" class="popquiz">
      ...
      <div class="hidden ifno-quiz">
        ...
        <div class="popquizbutton">
          <input id="showQuiz-{$qzno}" class="showQuiz hideSelf" type="button" value="{@button}" />
        </div>
        ...
        <div id="popquestion-{$qzno}" class="hidden">
          ...
          <p>
            <xsl:if test="not(@checkbox)">
              <input type="radio" name="popquiz" value="True" />
              True
              <br />
              <input type="radio" name="popquiz" value="False" />
              False
            </xsl:if>
            <xsl:for-each select="checkbox">
              <p>
                <input type="checkbox" name="popquiz" value="{@value}"/>
                <xsl:text/>
                <xsl:value-of select="." />
              </p>
            </xsl:for-each>
    ...
    <xsl:if test="@archive">
      <div class="archive">
        ...
        <p>
          ...
          <!--<img src="{$TSP}images/icons/archive-logo.jpg" alt="Archival Content"/>-->
        </p>
      </div>
    </xsl:if>
  ...
  <xsl:template match="flash-video">
    <xsl:variable name="STILL">
      <xsl:value-of select="@still" />
    </xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="contains ($VASERVER, 'www.hiv.va.gov') or contains ($VASERVER, 'www.hepatitis.va.gov')">
        <script type="text/javascript" src="swf/swfobject.js">
          <xsl:text/>
        </script>
        <div class="flash">
          <script type="text/javascript">
            ...
            var attributes = {};
            attributes.id = "myId";
            swfobject.embedSWF("swf/flvplayer.swf", "myContent", "
            <xsl:value-of select="$WIDTH" />
            ", "
            <xsl:value-of select="$HEIGHT" />
          ...
          <div id="myContent">
            <p>
              If you do not see a video above, you are either having trouble accessing the network, or do not have the Macromedia Flash Player installed. To download the Flash Player from Adobe's website:
      ...
      <xsl:otherwise>
        <script type="text/javascript" src="{$TSP}swf/swfobject.js">
          <xsl:text/>
        </script>
        <div class="flash">
          <script type="text/javascript">
  ...
  <xsl:template match="ref">
    <a href="#note{@no}" title="reference">
      ...
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="count(eref) = 1">
            <xsl:variable name="newREF">
              <xsl:call-template name="convertREF">
                <xsl:with-param name="FILE" select="eref/@id" />
              </xsl:call-template>
            </xsl:variable>
            (
            <a href="{$newREF}#note{eref/@no}" title="reference">
              <xsl:value-of select="eref/@no" />
            </a>
            )
          </xsl:when>
          <xsl:otherwise>
            <xsl:for-each select="eref">
              ...
              <xsl:choose>
                <xsl:when test="position() = 1">
                  (
                  <a href="{$newREF}#note{@no}" title="reference">
                    <xsl:value-of select="@no" />
                  </a>
                  ,
                </xsl:when>
  ...
  <xsl:template match="ilink[@selfref]">
    <xsl:variable name="newREF">
      <xsl:call-template name="convertREF">
        <xsl:with-param name="FILE" select="@selfref" />
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template match="ilink-mref">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <img src="{$TSP}images/icons/link-email.gif" alt="Opens your email to send a message" title="Opens your email to send a message" class="linkicons" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
    ...
    <xsl:choose>
      <xsl:when test="contains (@ref, '.mil') or contains (@ref, '.fed.us')">
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <img src="{$TSP}images/icons/link-external.gif" alt="Link will take you outside the VA website." title="Link will take you outside the VA website." class="linkicons" />
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
  ...
  <xsl:template match="ilink-kbref">
    ...
    <xsl:variable name="newREF">
      <xsl:call-template name="convertPost">
        <xsl:with-param name="FILE" select="'pr-kb-00'" />
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template match="ilink-hkb">
    ...
    <xsl:variable name="newREF">
      <xsl:call-template name="convertPost">
        <xsl:with-param name="FILE" select="'pr-kb-01'" />
      </xsl:call-template>
    </xsl:variable>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:for-each select="crumb">
            <xsl:choose>
              <xsl:when test="@link = 'index'"/>
              <xsl:when test="@link = 'va?page=we-00-00'"/>
              <xsl:when test="@link = 'vahiv?page=we-00-00'"/>
              <xsl:when test="@link = 'vahep?page=we-00-00'"/>
              <xsl:when test="position()=last()">
                <xsl:value-of select="@name" />
              </xsl:when>
              <xsl:otherwise>
    ...
    <p class="normal">
      ...
      <xsl:if test="related">
        <xsl:choose>
          <xsl:when test="$TS = 'true'">
            See
            <xsl:for-each select="related/see">
              <a href="{$LINKPATH}abbr-{@ref}#{.}">
                <xsl:value-of select="." />
              </a>
          ...
          <xsl:otherwise>
            See
            <xsl:for-each select="related/see">
              <a href="va?page=we-teamsite&#38;TS=abbr-{@ref}#{.}">
                <xsl:value-of select="." />
              </a>
              <xsl:if test="position()!=last()">;</xsl:if>
            </xsl:for-each>
        ...
        <xsl:choose>
          <xsl:when test="$TS = 'true'">
            <p>
              See
              <xsl:for-each select="related/see">
                <a href="{$LINKPATH}provider/glossary/{@ref}.asp#{.}">
                  <xsl:value-of select="." />
                </a>
                <xsl:if test="position()!=last()">;</xsl:if>
              </xsl:for-each>
          ...
          <xsl:otherwise>
            <p>
              See
              <xsl:for-each select="related/see">
                <a href="va?page=we-teamsite&#38;TS=provider/glossary/{@ref}#{.}">
                  <xsl:value-of select="." />
                </a>
                <xsl:if test="position()!=last()">;</xsl:if>
              </xsl:for-each>
  ...
  <xsl:template match="hot">
    <!-- CAN'T USE RED FONT ON VA SITES, PER PATTY -->
    <em>
      <xsl:apply-templates />
    </em>
  </xsl:template>
  ...
  <xsl:template match="input[@type='checkbox']">
    <xsl:if test="@label">
      <label for="{@name}" style="position: absolute; left: -9999px;">
  ...
  <xsl:template match="sec/sec/sec" mode="toc"/>
  ...
  <xsl:template match="product" mode="product">
    <xsl:variable name="PAGE">
      <xsl:choose>
        ...
        <xsl:when test="substring-after(id,'www') != ''">
          https://
          <xsl:value-of select="id" />
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template match="badge-wad">
    ...
    <textarea rows="3" cols="45">
      ...
      " width="160" height="160" alt="World AIDS Day" /&gt;&lt;/a&gt;&lt;/div&gt;
    </textarea>
    <xsl:apply-templates />
  </xsl:template>
  ...
  <xsl:template match="badge">
    ...
    <textarea rows="3" cols="45">
      ...
      "&gt;&lt;img src="
      <xsl:value-of select="$IMG" />
      " width="160" height="160" alt="Get Checked Campaign: Say yes to the test!" /&gt;&lt;/a&gt;&lt;/div&gt;
    </textarea>
    <xsl:apply-templates />
  </xsl:template>
    ...
    <xsl:if test="$PP = 'pr'">
      <div class="browse main">
        <xsl:choose>
          <xsl:when test="$TS = 'true'">
            <h2>
              <a href="{$LINKPATH}provider/topics/index.asp" query="#topicMenu" class="showDiv hideSelf">
                Browse
                <span class="italic" query="#topicMenu">by</span>
                Topic   +
              </a>
              <a href="{$LINKPATH}provider/topics/index.asp" query="#topicMenu" class="showDiv hideSelf hidden">
                Browse
                <span class="italic" query="#topicMenu">by</span>
                Topic   ^
              </a>
          ...
          <xsl:otherwise>
            <h2>
              <a href="va?page=we-teamsite&#38;TS=provider/topics/index" query="#topicMenu" class="showDiv hideSelf">
                Browse
                <span class="italic" query="#topicMenu">by</span>
                Topic   +
              </a>
              <a href="va?page=we-teamsite&#38;TS=provider/topics/index" query="#topicMenu" class="showDiv hideSelf hidden">
                Browse
                <span class="italic" query="#topicMenu">by</span>
                Topic   ^
              </a>
    ...
    <li>
      <xsl:choose>
        <xsl:when test="pgref">
          <xsl:variable name="newREF">
            <xsl:call-template name="convertREF">
              <xsl:with-param name="FILE" select="pgref" />
            </xsl:call-template>
          </xsl:variable>
          <a href="{$newREF}">
  ...
  <xsl:template match="item" mode="tools">
    <li>
      <img src="{$TSP}images/tools/{@type}.gif" width="20" height="20" border="0" alt="{@type} icon" />
      <xsl:apply-templates />
    </li>
  </xsl:template>
  ...
  <xsl:template match="item-word" mode="tools">
    <li>
      <a href="{$TSP}doc/{@ref}.doc">
        <img src="{$TSP}images/tools/word.gif" width="20" height="20" border="0" alt="Word icon" />
        Word document
      </a>
    </li>
  </xsl:template>
  ...
  <xsl:template match="item-pdf" mode="tools">
    <li>
      <a href="{$TSP}pdf/{@ref}.pdf">
        <img src="{$TSP}images/tools/pdf.gif" width="20" height="20" border="0" alt="PDF icon" />
        PDF version
      </a>
    </li>
  </xsl:template>
  ...
  <xsl:template match="item-ppt" mode="tools">
    <li>
      <a href="{$TSP}ppt/{@ref}.ppt">
        <img src="{$TSP}images/tools/powerpoint.gif" width="20" height="20" border="0" alt="PowerPoint icon" />
        PowerPoint slides
      </a>
    </li>
  </xsl:template>
  ...
  <xsl:template match="prev">
    <xsl:choose>
      <xsl:when test="@vahivref != ''">
        <xsl:variable name="newREF">
          <xsl:call-template name="convertREF">
            <xsl:with-param name="FILE" select="@vahivref" />
          </xsl:call-template>
        </xsl:variable>
        ...
        <xsl:if test="string-length(../toc-menu) = 0">
          <xsl:variable name="AnewREF">
            <xsl:call-template name="convertREF">
              <xsl:with-param name="FILE" select="/pg/page-0/contents/items/item/ilink-vahiv/@ref" />
            </xsl:call-template>
          </xsl:variable>
      ...
      <xsl:when test="@vahepref != ''">
        <xsl:variable name="newREF">
          <xsl:call-template name="convertREF">
            <xsl:with-param name="FILE" select="@vahepref" />
          </xsl:call-template>
        </xsl:variable>
        ...
        <xsl:if test="string-length(../toc-menu) = 0">
          <xsl:variable name="AnewREF">
            <xsl:call-template name="convertREF">
              <xsl:with-param name="FILE" select="/pg/page-0/contents/items/item/ilink-vahep/@ref" />
            </xsl:call-template>
          </xsl:variable>
          <a href="{$AnewREF}">Table of Contents</a>
          <xsl:text/>
  ...
  <xsl:template match="title-info" mode="page">
    <div class="pgtitle">
      ...
      <p class="byline">
        ...
        ,
        <xsl:apply-templates select="../display" mode="page" />
      </p>
    </div>
  </xsl:template>
  ...
  <xsl:template match="item" mode="toc">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$PGID != $PGALPHA">
        <xsl:variable name="newREF">
          <xsl:call-template name="convertREF">
            <xsl:with-param name="FILE" select="$PGALPHA" />
          </xsl:call-template>
        </xsl:variable>
        <li>
          <a href="{$newREF}">
  ...
  <xsl:template match="video" mode="mediaheader">
    <tr>
      <td width="100%" class="rowheader" colspan="2">
        <img src="{$TSP}images/global/clr_pxl.gif" width="1" height="1" border="0" alt="" />
        <xsl:value-of select="../@title" />
      </td>
      <xsl:if test="videoitem/flash">
        <td bgcolor="#ffffff" class="center">
          <img src="{$TSP}images/media/flash.gif" width="72" height="25" border="0" alt="Flash Player" />
        </td>
      </xsl:if>
      <xsl:if test="videoitem/wm">
        <td bgcolor="#ffffff">
          <img src="{$TSP}images/media/windows.gif" width="72" height="25" border="0" alt="Windows Media Player" />
        </td>
      </xsl:if>
      <xsl:if test="videoitem/rm">
        <td bgcolor="#ffffff">
          <img src="{$TSP}images/media/real.gif" width="72" height="25" border="0" alt="Real Media Player" />
        </td>
      </xsl:if>
      <xsl:if test="videoitem/qt">
        <td bgcolor="#ffffff">
          <img src="{$TSP}images/media/quicktime.gif" width="72" height="25" border="0" alt="Quicktime Player" />
        </td>
      </xsl:if>
  ...
  <xsl:template match="videoitem" mode="media">
    ...
    <tr bgcolor="#ffffff">
      <td align="top" class="mediatext" width="23">
        <img src="{$TSP}images/media/video.gif" width="20" height="25" border="0" alt="Video Clip" />
      </td>
      <td align="top" class="mediatext">
        <xsl:value-of select="@title" />
  ...
  <xsl:template match="transcript" mode="text">
    ...
    <br />
    <a href="{$newREF}" class="small">Transcript</a>
  </xsl:template>
  ...
  <xsl:template match="audioitem" mode="media">
    <tr bgcolor="#ffffff">
      <td align="top" class="mediatext" width="23">
        <img src="{$TSP}images/global/clr_pxl.gif" width="3" height="1" border="0" alt="" />
        <img src="{$TSP}images/media/audio.gif" width="20" height="25" border="0" alt="Audio Clip" />
      </td>
      <td align="top" class="mediatext">
        <xsl:value-of select="@title" />
        <xsl:apply-templates mode="text" />
  ...
  <xsl:template match="sec-first">
    <ul>
      <!-- since Insite links don't come wrapped in <ul> tag -->
      <xsl:apply-templates />
    </ul>
  </xsl:template>
      ...
      <xsl:choose>
        <xsl:when test="@class= 'intra'">
          <xsl:if test="$INTRA != ''">
            <li class="intra">
              <xsl:apply-templates />
              <span class="intratext">(VA Intranet-only content)</span>
            </li>
          </xsl:if>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:when test="$INTRA !=''">
        <xsl:choose>
          <xsl:when test="fref">
            <xsl:choose>
              <xsl:when test="contains ($REF, '.mil') or contains ($REF, '.fed.us')">
                <a href="https://{fref}" target="OffSite" title="{fname}">
              ...
              <xsl:when test="contains ($REF, '.gov')">
                <xsl:choose>
                  <xsl:when test="contains ($REF, 'va.gov')">
                    ...
                    <!-- <xsl:if test="fformat"> <xsl:text> [</xsl:text><xsl:value-of select="fformat" /><xsl:text>  </xsl:text><xsl:value-of select="fsize" /><xsl:text>]</xsl:text> </xsl:if>-->
                    <br />
                  </xsl:when>
                  <xsl:otherwise>
                    <a href="https://{fref}" target="OffSite" title="{fname}">
          ...
          <xsl:when test="fkbref">
            ...
            <a href="{$newREF}kb={fkbref}&#38;sec=00&#38;tp={ftopic}&#38;tpage={/pg/@id}" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          ...
          <xsl:when test="fpgref">
            <xsl:variable name="newREF">
              <xsl:call-template name="convertREF">
                <xsl:with-param name="FILE" select="fpgref" />
              </xsl:call-template>
            </xsl:variable>
            <a href="{$newREF}" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fpdfref">
            <a href="{$TSP}pdf/{fpdfref}.pdf" target="OffSite" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fpptref">
            <a href="{$TSP}ppt/{fpptref}.ppt" target="OffSite" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fppsref">
            <a href="{$TSP}ppt/{fppsref}.pps" target="OffSite" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fdocref">
            <a href="{$TSP}doc/{fdocref}.doc" target="OffSite" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fpubref">
            <xsl:variable name="newREF">
              <xsl:call-template name="convertPost">
                <xsl:with-param name="FILE" select="'home-pubmed'" />
              </xsl:call-template>
            </xsl:variable>
            <a href="{$newREF}uid={fpubref}" target="OffSite" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fepsref">
            <a href="{$TSP}images/{fepsref}.eps" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
          <xsl:when test="fwmref">
            <a href="{$WM}{fwmref}.wmv" title="{fname}">
              <xsl:value-of select="fname"/>
            </a>
            <br />
          </xsl:when>
    ...
    <div class="section-home">
      <xsl:if test="@title">
        <h3 class="homesec">
          ...
          <xsl:if test="@src">
            <img src="{$TSP}images/icons/{@src}" width="55" height="60" border="0" alt="{@title}" />
          </xsl:if>
        </h3>
      </xsl:if>
  ...
  <xsl:template match="drug" mode="header">
    ...
    <dl class="drug-class" onMouseOver="javascript:this.className+=' sfhover'" onMouseOut="javascript:this.className='drug-class'">
      <dt>
        Type of Drug:
        <xsl:value-of select="drug-class/class-name" />
      </dt>
      <dd id="class-def">
        <xsl:call-template name="drug-class">
          <xsl:with-param name="CLASS" select="drug-class/class-name/@id" />
        </xsl:call-template>
      </dd>
    </dl>
  </xsl:template>
  ...
  <xsl:template name="drug-class">
   ...
   -->
      ...
      <xsl:when test="$CLASS='CoPI'">
        <p>
          Fixed-dose combinations combine the most commonly prescribed doses of several medications into 1 pill. This pill is a combination of two drugs known as protease inhibitors.
        </p>
        <p>
          Once HIV has infected a cell and made copies of itself, it uses an enzyme called protease to process itself correctly so it can be released from the cell to infect other cells. Protease inhibitors work by blocking protease.
        </p>
      </xsl:when>
      <xsl:when test="$CLASS='CCR5'">
        ...
        <p>
          One of these is called a chemokine coreceptor. Drugs known as chemokine coreceptor antagonists block the virus from binding to the coreceptor.
        </p>
      </xsl:when>
  ...
  <xsl:template match="drug-name" mode="drug-name">
    ...
    <xsl:if test="combo-name or generic-name or abbreviations or other-names">
      ...
      <xsl:apply-templates select="combo-name" />
      <xsl:apply-templates select="other-names" />
      <xsl:apply-templates select="generic-name" />
  ...
  <xsl:template match="formulation">
    <xsl:if test="form-note">
      <p class="form-note">
        <xsl:value-of select="form-note" />
      </p>
    </xsl:if>
      ...
      <p>
        <xsl:choose>
          <xsl:when test="$TS = 'true'">
            ...
            <xsl:text/>
            <b>
              <xsl:value-of select="/pg/page-0/drug/drug-name/brand-name" />
            </b>
            <xsl:text/>
          </xsl:when>
          <xsl:otherwise>
            ...
            <xsl:text/>
            <b>
              <xsl:value-of select="document($TSXML)/pg/page-0/drug/drug-name/brand-name" />
              <xsl:text/>
            </b>
          </xsl:otherwise>
        </xsl:choose>
  ...
  <xsl:template match="with-drug">
    <xsl:text>+</xsl:text>
    <xsl:value-of select="with-formulation/amount" />
    <xsl:text/>
  ...
  <xsl:template match="with-drug-or">
    ...
    <xsl:value-of select="with-formulation-or/amount-or" />
    <xsl:text/>
    <xsl:apply-templates mode="with" />
  </xsl:template>
  ...
  <xsl:template match="drug-notes">
    ...
    <ul>
      <xsl:apply-templates mode="drug-notes" />
    </ul>
  </xsl:template>
  ...
  <xsl:template match="side-effects">
    ...
    <xsl:choose>
      <xsl:when test="../same-effects">
        <!-- only display link to chart once -->
      </xsl:when>
      <xsl:when test="$TS = 'true'">
        <p>
          See accompanying chart:
          <a href="{$LINKPATH}patient/drug-dosing-toolkit/side-effects-tips.asp">Tips for Common Side Effects</a>
        </p>
      </xsl:when>
      <xsl:otherwise>
        <p>
          See accompanying chart:
          <a href="va?page=we-teamsite&#38;TS=patient/drug-dosing-toolkit/side-effects-tips">Tips for Common Side Effects</a>
        </p>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="credit">
    <p class="photo-credit">
      ...
      <xsl:value-of select="/pg/page-0/drug/drug-name/brand-name" />
      is courtesy of
      <xsl:value-of select="/pg/page-0/drug/formulations/formulation/form-image/@credit" />
    </p>
  </xsl:template>
...
-->
  ...
  <xsl:template match="backto">
    <xsl:if test="$BACKTO != ''">
      <xsl:variable name="newREF">
        <xsl:call-template name="convertREF">
          <xsl:with-param name="FILE" select="$BACKTO" />
        </xsl:call-template>
      </xsl:variable>
      <p class="backto">
        <a href="{$newREF}">
  ...
  <xsl:template match="doc-source">
    <p class="bylinedocsource">
      <xsl:apply-templates />
    </p>
  </xsl:template>

  <xsl:template match="section-title">
    <p class="bylinesectiontitle">
      <xsl:apply-templates />
    </p>
  </xsl:template>
  ...
  <xsl:template match="display" mode="date">
    ...
    <p class="bylinetype">
      ...
      <xsl:value-of select="$LONGDATE" />
      <xsl:if test="../display-updated">
        <xsl:apply-templates select="../display-updated" mode="date" />
      </xsl:if>
    </p>
  </xsl:template>
  ...
  <xsl:template match="posted">
    ...
    <xsl:if test="../updated !=''">
      ; updated
      <xsl:apply-templates select="../updated" mode="date" />
    </xsl:if>
    <xsl:if test="../reviewed !=''">
      ; reviewed
      <xsl:apply-templates select="../reviewed" mode="date" />
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="rollimg">
    ...
    <xsl:choose>
      <xsl:when test="@starter">
        <a onmouseover="javascript:document.{@name}.src='{$TSP}images/{@rollover}'" onmouseout="javascript:document.{@name}.src='{$TSP}images/{@src}'" onmouseup="javascript:document.{@name}.src='{$TSP}images/{@mouseup}'">
          <img src="{$TSP}images/{@starter}" name="{@name}" width="{@width}" height="{@height}" style="border:0;" alt="{@alt}" />
        </a>
        <xsl:if test="@caption">
          <p class="caption">
            <xsl:value-of select="@caption" />
      ...
      <xsl:otherwise>
        <a onmouseover="javascript:document.{@name}.src='{$TSP}images/{@rollover}'">
          <img src="{$TSP}images/{@src}" name="{@name}" width="{@width}" height="{@height}" alt="{@alt}" />
        </a>
        <br />
        <xsl:if test="@caption">
          <p class="caption">
  ...
  <xsl:template match="item-img">
    <tr>
      <td valign="top">
        <input type="checkbox" name="" value="{@value}" id="{@value}" />
      </td>
      <td>
        <label for="{@value}">
          <xsl:apply-templates />
  ...
  <xsl:template match="algorithm-yale">
    <script type="text/javascript" src="{$TSP}styles/livertests.js">
      <xsl:text/>
    </script>
  ...
  <xsl:template match="algorithm">
    ...
    <script type="text/javascript" src="{$TSP}{@script}"/>
  ...
  <xsl:template match="area">
    ...
    <xsl:choose>
      <xsl:when test="@jump">
        <area alt="{@alt}" shape="{@shape}" coords="{@coords}" href="{$newREF}{@jump}"/>
      </xsl:when>
      <xsl:otherwise>
    ...
    <p>
      <xsl:choose>
        <xsl:when test="$TS = 'true' or contains ($VASERVER, 'www.hiv.va.gov') or contains ($VASERVER, 'www.hepatitis.va.gov')">
          <a onClick="history.go(-1)" onMouseOver="image1.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_previous_on.png';" onMouseOut="image1.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_previous.png';">
            <img name="image1" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_previous.png" alt="Previous" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/start.asp" onMouseOver="image2.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_start_on.png';" onMouseOut="image2.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_start.png';">
            <img name="image2" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_start.png" alt="Start" border="0" />
          </a>
          <a onMouseOver="image3.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic_on.png';" href="/HEPATITIS/provider/tools/algorithm-hcc/diagnosis_start.asp" onMouseOut="image3.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic.png';">
            <img name="image3" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic.png" alt="Diagnostic Algorithm" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/tx_start.asp" onMouseOver="image4.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_on.png';" onMouseOut="image4.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment.png';">
            <img name="image4" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment.png" alt="Treatment Algorithm" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/child_pugh_chart.asp" onMouseOver="image5.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh_on.png';" onMouseOut="image5.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh.png';">
            <img name="image5" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh.png" alt="Child-Pugh Chart" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/hcc_guide.asp" onMouseOver="image6.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines_on.png';" onMouseOut="image6.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines.png';">
            <img name="image6" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines.png" alt="Treatment Guidelines" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/hcc_contacts.asp" onMouseOver="image7.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info_on.png';" onMouseOut="image7.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info.png';">
            <img name="image7" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info.png" alt="Contact Info" border="0" />
          </a>
          <a href="/HEPATITIS/provider/tools/algorithm-hcc/hcc_help.asp" onMouseOver="image8.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_help_on.png';" onMouseOut="image8.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_help.png';">
            <img name="image8" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_help.png" alt="Help" border="0" />
          </a>
        </xsl:when>
        ...
        <xsl:otherwise>
          ...
          <a href="{$newREF}provider/tools/algorithm-hcc/start" onMouseOver="image2.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_start_on.png';" onMouseOut="image2.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_start.png';">
            <img name="image2" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_start.png" alt="Start" border="0" />
          </a>
          <a onMouseOver="image3.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic_on.png';" href="{$newREF}provider/tools/algorithm-hcc/diagnosis_start" onMouseOut="image3.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic.png';">
            <img name="image3" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_diagnostic.png" alt="Diagnostic Algorithm" border="0" />
          </a>
          <a href="{$newREF}provider/tools/algorithm-hcc/tx_start" onMouseOver="image4.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_on.png';" onMouseOut="image4.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment.png';">
            <img name="image4" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment.png" alt="Treatment Algorithm" border="0" />
          </a>
          <a href="{$newREF}provider/tools/algorithm-hcc/child_pugh_chart" onMouseOver="image5.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh_on.png';" onMouseOut="image5.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh.png';">
            <img name="image5" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_childpugh.png" alt="Child-Pugh Chart" border="0" />
          </a>
          <a href="{$newREF}provider/tools/algorithm-hcc/hcc_guide" onMouseOver="image6.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines_on.png';" onMouseOut="image6.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines.png';">
            <img name="image6" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_treatment_guidelines.png" alt="Treatment Guidelines" border="0" />
          </a>
          <a href="{$newREF}provider/tools/algorithm-hcc/hcc_contacts" onMouseOver="image7.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info_on.png';" onMouseOut="image7.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info.png';">
            <img name="image7" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_contact_info.png" border="0" />
          </a>
          <a href="{$newREF}provider/tools/algorithm-hcc/hcc_help" onMouseOver="image8.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_help_on.png';" onMouseOut="image8.src='{$TSP}images/provider/tools/algorithm-hcc/hccimages_help.png';">
            <img name="image8" src="{$TSP}images/provider/tools/algorithm-hcc/hccimages_help.png" alt="Help" border="0" />
          </a>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="knowmore-widget">
    <a href="https://www.cdc.gov/hepatitis/riskassessment/?s_cid=bb-dvh-hra_008" title="VIRAL HEPATITIS. ARE YOU AT RISK? Take this online assessment to see if you're at risk. https://www.cdc.gov/hepatitis/riskassessment/">
  ...
  <xsl:template match="testingday-widget">
    <div class="related-image">
      <a href="https://hivtest.cdc.gov/press_files/Default.aspx?s_cid=hivtesting_011" title="Take the Test, Take Control. National HIV Testing Day � 6/27/2010">
        <img src="https://www.cdc.gov/images/campaigns/hivtesting/2010/TakeTheTest_100x120.gif" style="width:100px; height:120px; border:0px;" alt="Take the Test, Take Control. National HIV Testing Day � 6/27/2010" />
      </a>
    </div>
  </xsl:template>
  ...
  <xsl:template match="accordion">
    <div class="accordionsexpand">
      <xsl:if test="@expand">
        <button class="accordionall accordionexpand">All</button>
      </xsl:if>
      <xsl:apply-templates />
    </div>
  </xsl:template>
    ...
    <section class="hero">
      ...
      <footer>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <nav>
              <xsl:if test="menu/@class">
                <xsl:attribute name="class">
                  <xsl:value-of select="menu/@class" />
                </xsl:attribute>
              </xsl:if>
              <h2>Main Menu</h2>
  ...
  <xsl:template match="li" mode="t4">
    <li>
      <xsl:choose>
        <xsl:when test="../../menu[@class='listing']">
          <xsl:apply-templates />
        </xsl:when>
        <xsl:otherwise>
          <xsl:if test="title">
  ...
  <xsl:template match="listing">
    <nav class="listing">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <h2 class="visually-hidden">Lesson Menu</h2>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*[not(self::title)]" />
    </nav>
